...
**当前状态：** 年末 SOC 的 1200/6000 kWh 方案均已登记，但尚未选定唯一执行口径；Cursor 可完成不依赖该数值的代码准备与前置产物归档，**不得**启动正式全年运行，直到队长在两案中确认一案。
...
3. 年末边界见第 1.1 节的 A/B 方案。无论选哪一案，所有年度策略和敏感性必须使用同一边界并在论文中说明；不能让各策略自然落在不同年末 SOC 后直接比较成本。
...
### 1.1 年末 SOC：待队长确定的 A/B 方案记录

| 方案 | 共同边界 | 已记录的理由 | 比较含义 |
| --- | --- | --- | --- |
| A：Q2 对齐 | \(E_{2025-12-31,144}=1200\) kWh | 与 Q2 已采用的年度终端边界一致；所有 Q3 策略消耗同一部分期初库存。 | 年度运行成本含从 6000 降至 1200 kWh 的共同库存变化，适合与当前 Q2 口径并列。 |
| B：能量中性 | \(E_{2025-12-31,144}=6000\) kWh | 年初、年末库存相同，避免将 4800 kWh 期初能量的消耗计作不同策略的免费收益。 | 更适合把各策略成本差解释为同一净储能库存条件下的调度差异。 |

两案互斥，均不是题设直接给定的事实。后续讨论确定主方案后，另一案应保留为年末边界敏感性；在此之前不得导出或解释全年正式成本。
...
新增全年测试至少验证：全年无日期缺失/重复、365 日 × 144 时段完整、每条路径跨日 SOC 连续、年初 6000/年末为队长选定的共同 A/B 边界、唯一运行键无重复、预测历史截止日不晚于决策日、各口径不混用。
