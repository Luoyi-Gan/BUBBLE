\noindent 信息结构与预测映射。设一天含 $T=144$ 个 10 分钟时段，$\Delta t=1/6\,\mathrm{h}$。预测更新时刻为 $0{:}00$、$6{:}00$、$12{:}00$ 和 $18{:}00$，记其集合为 $\mathcal U$。以 $m_\tau$ 表示更新时刻 $\tau$ 后首个可调整时段，则
\begin{equation}
...
\noindent 计划调整的结算模型。记 $g_{d,t}^{0}$ 为当天 $0{:}00$ 制定的普通购电承诺，$g_{d,t}$ 为该时段到来前最后一次有效调整后的承诺，$e_{d,t}$ 为实时紧急购电量。对同一时段，结算只相对 $g_{d,t}^{0}$ 进行一次：
\begin{equation}
...
\noindent 滚动购电与储能优化模型。在更新时刻 $\tau$，令剩余时段集合为 $\mathcal T_\tau=\{m_\tau,\ldots,T\}$。以真实当前储电量 $E_{d,m_\tau-1}$ 为初始状态，最小化剩余时域的结算费、紧急购电费和下一日储电量价值：
\begin{equation}
...
\begin{alignat}{2}
x_{d,t}+e_{d,t}+\widehat P_{d,t}^{\tau}-w_{d,t}+r_{d,t}&=\widehat L_{d,t}+c_{d,t}, &&\quad t\in\mathcal T_\tau,\label{eq:q3-balance}\\
E_{d,t}&=E_{d,t-1}+\eta_c c_{d,t}-\frac{r_{d,t}}{\eta_d}, &&\quad t\in\mathcal T_\tau,\label{eq:q3-soc}\\
0&\le x_{d,t}\le g_{d,t}, &&\quad t\in\mathcal T_\tau,\nonumber\\[-2pt]
0&\le e_{d,t}, &&\quad t\in\mathcal T_\tau,\label{eq:q3-normal-limit}\\
0&\le c_{d,t},r_{d,t}\le5000\Delta t, &&\quad t\in\mathcal T_\tau,\nonumber\\[-2pt]
0&\le w_{d,t}\le\widehat P_{d,t}^{\tau}, &&\quad t\in\mathcal T_\tau,\label{eq:q3-power-limit}\\
1200&\le E_{d,t}\le10800, &&\quad \eta_c=\eta_d=\sqrt{0.9}.\label{eq:q3-soc-limit}
\end{alignat}
...
\noindent 信息价值触发机制与对照策略。为判断某次新预报是否值得用于调整，分别在“锁定原承诺”和“允许调整”下求解式\eqref{eq:q3-objective}：
\begin{align}
...
\label{eq:q3-voi}
\end{align}
当 $\operatorname{VoI}_{\tau}>\varepsilon$ 时实施新承诺，否则维持 $g^{\mathrm{pre}}$；$\varepsilon=0.01$ 元仅用于屏蔽数值舍入引起的无意义微调。本文将日初计划不再调整的基准称为“不调整策略（M0）”，将三个更新点均评估并按式\eqref{eq:q3-voi}实施的方案称为“全时点价值触发策略（M1/M6）”；另以 $M_6$、$M_{12}$、$M_{18}$ 表示只开放相应单一更新点的消融策略。它们共同回答后续预报是否有价值，以及何时更新最有效。
